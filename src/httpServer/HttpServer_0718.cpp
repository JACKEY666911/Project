...
void post_device_cb(struct evhttp_request *req, void *arg)
{
	...
	if(results["flag"].asInt() == DEVICE_MANAGE)
	{	
		device_query_update(req, results);
	}
	else if(results["flag"].asInt() == CLOCK_MANAGE)
	{
		clock_manage(req, results);
	}
	else
	{
		return ;
...
void post_client_cb(struct evhttp_request *req, void *arg)
{
	...
	else if(results["flag"].asInt() == PC_CLOCK_QUERY)
	{
		clock_manage(req, results);
	}
	else
...
void user_manage(struct evhttp_request *req, const Value &results)
{
	...
	switch (results["mode"].asInt())
	{
		case USER_INSERT:
			...
			break;
		case USER_QUERY:
			ret = client.table2_queryAllMsg(root);
			cout << ret << endl;
...
void device_query_update(struct evhttp_request *req, const Value &results)
{
	...
	printf("client connected!\n");
	forDevice device("test10.db");
	device.CreateForm();

	int code 			= results["code"].asInt();
	int id	 			= results["id"].asUInt();
	int fingerID 		= results["fingerID"].asInt();

	switch (results["mode"].asInt())
	{
		case USERMSG_QUERY:
			device.table2_queryAllMsg(root);
			response = writer.write(root);
			response_post(req, response.c_str());
			printf("USER_QUERY!\n");
			break;
		case FINGER_INSERT:
			ret = device.table2_update(id, fingerID);
			if(ret != 0)
			{
				response = SQLITE_ERR;
			}
...
void clock_manage(struct evhttp_request *req, const Value &results)
{
	...
	struct insert clockMesg ={0};
	
	int code = results["code"].asInt();
	int mode = results["mode"].asInt();
	clockMesg.id = results["id"].asUInt();
	clockMesg.name = results["name"].asString();
	...
	switch (results["mode"].asInt())
	{
		case CLOCK_INSERT:
			...
			printf("DEVICE_INSERT!\n");
			response_post(req, response.c_str());
			break;
		case CLOCK_QUERY:
			device.queryMsg(code, root, mode);
			response = writer.write(root);
			response_post(req, response.c_str());
			printf("DEVICE_QUERY!\n");
			break;
		case EACH_MONTH:
			client.queryMsg(code, root, mode);
			response = writer.write(root);
			response_post(req, response.c_str());
			printf("PC_CLIENT1_QUERY!\n");
			break;	
		case EACH_DAY:
			client.queryMsg(code, root, mode);
			response = writer.write(root);
			response_post(req, response.c_str());
			printf("PC_CLIENT2_QUERY!\n");
			break;		
		default:
			break;
	}
